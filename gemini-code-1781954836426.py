...
/* Hide any icon (SVG or Material symbol) inside the upload button to prevent overlapping */
...
/* Force ALL text inside an expander (markdown, bold, plain) to be dark.
   CRITICAL FIX: Exclude the material icon spans so the arrow doesn't turn into raw text! */
[data-testid="stExpander"] p,
[data-testid="stExpander"] span:not([data-testid="stIconMaterial"]):not(.material-symbols-rounded),
...
<script>
function fixUI() {
    // 1. Hide the material icon/SVG inside the upload button (prevents overlapping "upload" text)
    document.querySelectorAll('[data-testid="stFileUploaderDropzone"] button [data-testid="stIconMaterial"], [data-testid="stFileUploaderDropzone"] button .material-symbols-rounded, [data-testid="stFileUploaderDropzone"] button svg').forEach(el => {
        el.style.display = 'none';
        el.innerHTML = ''; // Strips out the text ligature entirely
    });
