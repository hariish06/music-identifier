...
/* ── SIDEBAR IS FIXED — collapse/reopen control removed entirely ── */
[data-testid="collapsedControl"] {
    display: none !important;
    visibility: hidden !important;
}

/* Hide the sidebar's own collapse button so it can't be closed */
...
.stTabs [data-baseweb="tab"] {
    background: transparent !important; color: #6b7f6b !important;
    border-radius: 7px !important; font-size: 0.83rem !important; font-weight: 600 !important;
}
...
/* The dropzone is a flex row: [icon+text] ... [Browse files button].
   Without explicit wrap/shrink rules, it overlaps when the container
   width changes (e.g. sidebar opening/closing). Force it to wrap and
   give each side room to breathe instead of stacking on top of itself. */
...
/* Icon + "Drag and drop..." text wrapper — let it shrink and wrap text
   instead of forcing the button into the same line/space */
...
/* "Browse files" button — fixed, never shrinks or gets overlapped */
...
/* Relabel the button from "Browse files" to "Find files" and make sure
   the new label isn't drawn on top of the old one */
...
/* Uploaded-file row (filename chip below dropzone) — same overlap risk */
...
/* The summary's first inner wrapper holds the label text — let it
   shrink/truncate instead of overlapping the toggle arrow */
...
/* Streamlit's expander chevron is a Material-Symbols ligature icon.
   When that font fails to load it falls back to showing the raw
   ligature text (e.g. "keyboard_arrow_down"), which overlaps the
   filename. Hide that text and draw a plain CSS triangle instead so
   it never depends on an external icon font. */
...
/* ── PLAIN HELPER/CAPTION TEXT (e.g. "Supports WAV, MP3...",
   "Upload multiple audio files...") was rendering near-invisible —
   force it fully dark and opaque everywhere in the main content area,
   not just inside expanders. ── */
...
/* ── HIDE CHROME — sidebar is fixed, no reopen control needed ── */
...
if st.session_state.mode == "Single":
    ...
    st.write("Supports WAV, MP3, OGG, FLAC, M4A")
